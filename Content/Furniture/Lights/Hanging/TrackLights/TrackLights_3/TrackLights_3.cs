...
namespace SquintlysFurnitureMod.Content.Furniture.Lights.Hanging.TrackLights.TrackLights_3
{
    public class TrackLights_3 : ModTile
    {
        public enum StyleID
        {
            TatteredTrackLight, //0
            RepairedTrackLight, //0
        }
        ...
        public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b)
        {
            ...
            switch (style)
            {
                case StyleID.RepairedTrackLight:
                    r = 1f;
                    g = .95f;
                    b = .90f;
                    break;

                case StyleID.TatteredTrackLight:
                    r = 1f;
                    g = .75f;
                    b = .75f;
                    break; //warm dim
        ...
        public override void AdjustMultiTileVineParameters(int i, int j, ref float? overrideWindCycle, ref float windPushPowerX, ref float windPushPowerY, ref bool dontRotateTopTiles, ref float totalWindMultiplier, ref Texture2D glowTexture, ref Color glowColor)
        {
            ...
            switch (style)
            {
                case StyleID.TatteredTrackLight: //Default
                case StyleID.RepairedTrackLight:
                    if (frameX < 36)
                    {
                        overrideWindCycle = 0f;
                    }
                    if (frameX > 106 & frameX < 144)
                    {
                        overrideWindCycle = 0f;
                    }
                    else
