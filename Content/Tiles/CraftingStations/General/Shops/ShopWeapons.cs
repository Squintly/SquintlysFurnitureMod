...
public class ShopWeapons : ModTile
{
    ...
    public override bool RightClick(int i, int j)
    {
        ...
        if (!shiftPressed)
        {
            for (int x = topX; x < topX + 5; x++)
            {
                ...
                {
                    Main.tile[x, y].TileType = (ushort)ModContent.TileType<ShopBooks>();
                }
            }
        }
        ...
        if (shiftPressed)
        {
            for (int x = topX; x < topX + 5; x++)
            {
                for (int y = topY; y < topY + 4; y++)
                {
                    Main.tile[x, y].TileType = (ushort)ModContent.TileType<ShopVeggies>();
                }
            }
        }
