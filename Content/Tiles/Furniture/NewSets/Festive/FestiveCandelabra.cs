...
{
	internal class FestiveCandelabra : ModTile
	{
		...
		public override void SetStaticDefaults() {
            ...
            AddMapEntry(new Color(79, 71, 58), Language.GetText("Festive Candelabra"));

			if (!Main.dedServ) {
				flameTexture = ModContent.Request<Texture2D>("SquintlysFurnitureMod/Content/Tiles/Furniture/NewSets/Festive/FestiveCandelabra_Flame");
			}
		}

        public override void KillMultiTile(int x, int y, int frameX, int frameY)
        {
            Item.NewItem(new EntitySource_TileBreak(x, y), x * 16, y * 16, 48, 32, ModContent.ItemType<Items.Furniture.NewSets.Festive.FestiveCandelabraItem>());
        }
  ...
  //          }
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch) {
            ...
            for (int c = 0; c < 7; c++)
            {
                float shakeX = Utils.RandomInt(ref randSeed, -5, 11) * 0.05f;
