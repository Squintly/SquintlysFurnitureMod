...
namespace SquintlysFurnitureMod.Content.Tiles.Furniture.NewSets.Festive
{
    internal class FestiveChandelier : ModTile
    {
        ...
        public override void SetStaticDefaults()
        {
            ...
            AddMapEntry(new Color(12, 69, 6), base.CreateMapEntryName("Festive Chandelier")); ;

            if (!Main.dedServ)
            {
                flameTexture = ModContent.Request<Texture2D>("SquintlysFurnitureMod/Content/Tiles/Furniture/NewSets/Festive/FestiveChandelier_Flame"); // We could also reuse Main.FlameTexture[] textures, but using our own texture is nice.
            }
        }

        public override void KillMultiTile(int i, int j, int frameX, int frameY)
        {
            Item.NewItem(new EntitySource_TileBreak(i, j), i * 16, j * 16, 16, 32, ModContent.ItemType<FestiveChandelierItem>());
        }
