...
public class ShopHeartfelt : ModTile
{
    public override void SetStaticDefaults()
    {
        ...
        AddMapEntry(new Color(200, 200, 200), Language.GetText("SeasonalShop"));
    ...
    public override bool RightClick(int i, int j)
    {
        ...
        if (!shiftPressed)
        {
            for (int x = topX; x < topX + 5; x++)
            {
                ...
                {
                    Main.tile[x, y].TileType = (ushort)ModContent.TileType<ShopSpooky>();
                }
            }
        }
        ...
        if (shiftPressed)
        {
            for (int x = topX; x < topX + 5; x++)
            {
                for (int y = topY; y < topY + 4; y++)
                {
                    Main.tile[x, y].TileType = (ushort)ModContent.TileType<ShopCrimbo>();
                }
            }
        }
