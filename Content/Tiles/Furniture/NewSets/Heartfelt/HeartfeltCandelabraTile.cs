...
{
	internal class HeartfeltCandelabraTile : ModTile
	{
		...
		public override void SetStaticDefaults() {
            ...
            AddMapEntry(new Color(252, 3, 3), Language.GetText("Heartfelt Candelabra"));

			if (!Main.dedServ) {
				flameTexture = ModContent.Request<Texture2D>("SquintlysFurnitureMod/Content/Tiles/Furniture/NewSets/Heartfelt/HeartfeltCandelabraTile_Flame");
			}
		}

        public override void KillMultiTile(int x, int y, int frameX, int frameY)
        {
            Item.NewItem(new EntitySource_TileBreak(x, y), x * 16, y * 16, 48, 32, ModContent.ItemType<HeartfeltCandelabra>());
        }
  ...
  //          }
		...
		public override void PostDraw(int i, int j, SpriteBatch spriteBatch) {
            ...
            for (int c = 0; c < 2; c++)
            {
                float shakeX = Utils.RandomInt(ref randSeed, -5, 11) * 0.01f;
